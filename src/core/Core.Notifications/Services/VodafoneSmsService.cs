...
public class VodafoneSmsService : ISmsService
{
    ...
    public async Task<SmsResponse> SendAsync(SmsMessage message)
    {
        try
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                _logger.Information(string.Format("SMS sent successfully to {To}, Message ID: {MessageId}",
            ...
            {
                _logger.Warning(string.Format("Failed to send SMS to {To}. Status: {StatusCode}, Response: {Response}",
        ...
        {
            _logger.Error(ex, string.Format("Error sending SMS to {To}", message.To));
    ...
    public async Task<decimal> GetBalanceAsync()
    {
        ...
        _logger.Warning("Balance check not implemented for Vodafone SMS API");
        return 0;
    }
    ...
    public async Task<SmsResponse> GetStatusAsync(string messageId)
    {
        ...
        {
            _logger.Error(ex, string.Format("Error getting SMS status for {MessageId}", messageId));
    ...
    private async Task AcquireTokenAsync()
    {
        try
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                _logger.Information("Successfully acquired Vodafone API token");
            }
            else
            {
                _logger.Warning(string.Format("Failed to acquire Vodafone API token. Status: {StatusCode}, Response: {Response}",
        ...
        {
            _logger.Error(ex, "Error acquiring Vodafone API token");
            throw;
        }
    }
